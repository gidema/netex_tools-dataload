...
public abstract class AbstractNetexListDataloadJob<T, S> {
    ...
    private final ItemProcessor<T, List<S>> processor;
     ...
     */
    @Bean
    ItemWriter<List<S>> writer() {
        var jpaWriter = new JpaItemWriter<S>();
        jpaWriter.setEntityManagerFactory(entityManagerFactory);
        return new ItemListItemWriter<>(jpaWriter);
    }
    ...
    Step importStep(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
        return new StepBuilder("importStep", jobRepository)
            .<T, List<S>>chunk(1000, transactionManager)
